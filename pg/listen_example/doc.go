...
mechanism to avoid polling the database while waiting for more work to arrive.
    ...
    func waitForNotification(l *pg.Listener) {
        for {
            select {
                case <-l.Notify:
    ...
    func main() {
        ...
        reportProblem := func(ev pg.ListenerEventType, err error) {
            if err != nil {
                fmt.Println(err.Error())
            }
        }

        listener := pg.NewListener(conninfo, 10 * time.Second, time.Minute, reportProblem)
